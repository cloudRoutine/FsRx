...
module AsyncSeq = 
  ...
  type AsyncSeqBuilder with
    ...
    member x.For(seq:seq<'T>, action:'T -> AsyncSeq<'TResult>) = 
        ...
        (   x.While((fun () ->  enum.MoveNext())                                , 
                                x.Delay( fun () -> action enum.Current))        , 
  ...
  let map f (input : AsyncSeq<'T>) : AsyncSeq<'a>= 
  ...
  let iter f (input : AsyncSeq<'T>) : Async<unit> = 
  ...
  let toBlockingSeq (input : AsyncSeq<'T>) : seq<'T> = 
